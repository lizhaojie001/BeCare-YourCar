...
@interface ArticleTypeFourViewController()
@property RLMResults   * data;
@end
@implementation ArticleTypeFourViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self firstComein];
...
-(void)pullUpRefresh{
    ...
    [params setValue:@(82) forKey:@"class"];
    ...
    [self.manager zj_GET:baseURL withParams:params successComplete:^(id response) {
        if (response !=nil && [response isKindOfClass:[NSDictionary class]]) {
            if ([[response valueForKey:@"returncode"] intValue] == 0) {
                ...
                if (is) {
                    self.data = [Article_hitlist_data objectsWhere:@" class_name == %@",@"用车养车"];
